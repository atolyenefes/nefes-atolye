...
async function loadArchivedNotes() {
    ...
    if (error) {
        ...
        // Supabase'den gelen veriyi eski JS yapına uygun hale getiriyoruz
        allNotes = data.map(row => {
            ...
            return {
                id: row.id, // Silme işlemi için ID'yi tutuyoruz
                nickname: row.nickname,
                note: row.note,
                date: tarihMuhu,
                ip: row.ip_address,
                forceNewPage: row.force_new_page
            };
        });
    } else {
        allNotes = [];
    ...
    calculateBookLayout();
    currentSpread = maxSpread; // Kitap açıldığında son sayfayı göster
    renderCurrentSpread();
}
